...
# Reading in data
mlb_orig <- read.csv(file = "MLB_OBP3.csv", header = TRUE)
...
predict.lm (mlb_mlr,newdata=,
            mlb_orig[which(mlb_orig$last_name == "Votto" & mlb_orig$year == "2015"),],
            interval="prediction", level = 0.95)
# Real Stats: 0.459

# Try prediction - Shin-Soo Choo(2018)
predict.lm (mlb_mlr,newdata=,
            mlb_orig[which(mlb_orig$last_name == "Choo" & mlb_orig$year == "2018"),],
            interval="prediction", level = 0.95)
# Real Stats: 0.376
...
# --------------------------------------------------------
# Try to predict 2019 using 2015 ~ 2018
mlb_2019 <- filter(mlb_orig, year == "2019")


mlb_prev <- filter(mlb_orig, year != "2019")

mlb_mlr_2 <- lm(on_base_percent ~ barrel_batted_rate +
...
# Std.Residual histogram (normality)
std2 <- stdres(mlb_mlr_2)
hist(std2, main = "Histogram of Std. Residuals", xlab = "Standardized Residuals")

ks.test(std2,"pnorm")

# Residual plots
qplot(x = mlb_mlr_2$fitted.values, y = mlb_mlr_2$residuals, 
      data = mlb_mlr_2, geom = "point", xlab = "Fitted Values", 
      ylab = "Residuals") + geom_hline(aes(yintercept=0))

bptest(mlb_mlr_2)

pred2019 <- predict.lm(mlb_mlr_2, newdata = mlb_2019, interval = "prediction", level = 0.95)

bias2019 <- pred2019[,1] - mlb_2019$on_base_percent

mean(bias2019)

rmse2019 <- sqrt(mean((pred2019[,1] - mlb_2019$on_base_percent)^2))
rmse2019

cov2019 <- mean(pred2019[,2] < mlb_2019$on_base_percent & pred2019[,3] > mlb_2019$on_base_percent)
cov2019

wid2019 <- mean(pred2019[,3]-pred2019[,2])
wid2019
